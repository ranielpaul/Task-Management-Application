...
- `Backend/routers/tasks.py` — Task REST endpoints under `/api/tasks`:
  - `GET /api/tasks` — list tasks, optionally filtered by `search` and/or `is_complete`.
  - `POST /api/tasks` — create a task.
  - `GET /api/tasks/{task_id}` — fetch one task.
  - `PUT /api/tasks/{task_id}` — update title, description, and/or `is_complete`.
  - `PATCH /api/tasks/{task_id}/toggle` — toggle `is_complete`.
...
- `Backend/core/models.py` — SQLAlchemy ORM models. The `Task` model maps to a table whose name is controlled by the `DATABASE_TABLE_NAME` env var (defaults to `tasks`). The model has `id` (UUID), `title`, `description`, `is_complete` (boolean), and `created_at`.
...
- `src/api/TaskAPI.js` — Exports `taskApi` with all task functions: fetch (with search/isComplete filters), create, update, delete, toggle complete, search, and filter helpers.
- `src/hooks/useTaskFilters.js` — React hook owning the search and filter UI state (`searchTerm`, `appliedSearchTerm`, `selectedComplete`) and exposing `searchTasks`, `applySearch`, `filterByComplete`, plus a derived `filters` object for the API.
...
- `src/pages/home/components/Header.jsx` — Search bar plus the Status filter dropdown.
...
- `src/pages/home/components/TaskForm.jsx` — Add/Edit task modal with title, description, and completed checkbox.
